...
OBJECTS_plus_agent = ['sticks', 'axe', 'hammer', 'rock', 'tree', 'bread', 'house', 'wheat', 'agent']
OBJECT_RATIOS = [1, 1, 1, 1, 1, 1, 1, 1]
OBJECT_PROBS = [x / sum(OBJECT_RATIOS) for x in OBJECT_RATIOS]

CPV_COLORS = [(45, 82, 160), (255, 102, 102), (204, 204, 0), (211, 211, 211), (34, 133, 34), (0, 215, 255),
              (153, 52, 255), (10, 215, 100), (0, 0, 255)]


CPV_COLORS_M = np.asarray(CPV_COLORS)


COLORS = [(110, 69, 39), (255, 105, 180), (100, 100, 200), (100, 100, 100), (0, 128, 0), (205, 133, 63), (197, 91, 97),
          (240, 230, 140)]
COLORS_A = [(110, 69, 39), (255, 105, 180), (100, 100, 200), (100, 100, 100), (0, 128, 0), (205, 133, 63), (197, 91, 97),
...
COLORS_H2 = np.asarray([(110, 69, 39), (255, 105, 180), (100, 100, 200)])

COLORS_N_M = np.asarray(COLORS_N)
# COLORS_A_M = np.asarray(COLORS_A).reshape((3,3,3))
COLORS_A_M = CPV_COLORS_M.reshape((3,3,3))
OBJECT_ENCODING = []
for i in range(9):
    zeros = [0 for _ in range(9)]
    zeros[i] = 1
    OBJECT_ENCODING.append(zeros)
OBJECT_ENCODING_M = np.asarray(OBJECT_ENCODING).reshape(3,3,9)
# print(OBJECT_ENCODING_M)
OBJECTS_M = np.asarray([0,1,2,3,4,5,6,7,8]).reshape(3,3)
# print(OBJECTS_M)
# print(COLORS_A_M.shape)
# print(COLORS_A_M)
...
COLORS_rgba = [(45/255.0, 82/255.0, 160/255.0, .9),
                   (255/255.0, 102/255.0, 102/255.0, .9),
                   (204/255.0, 204/255.0, 0/255.0, .9),
                   (211/255.0, 211/255.0, 211/255.0, .9),
                   (34/255.0, 133/255.0, 34/255.0, .9),
                   (0/255.0, 215/255.0, 255/255.0, .9),
                   (153/255.0, 52/255.0, 255/255.0, .9),
                   (10/255.0, 215/255.0, 100/255.0, .9),
                   (0/255.0, 0/255.0, 255/255.0, .9)]
...
class CraftingWorldEnvAltObs(gym.GoalEnv):
    ...
    def __init__(self, size=(STATE_W,STATE_H), fixed_init_state=None, max_steps=MAX_STEPS, store_gif=False, render_save_rate=1, task_list=TASK_LIST,
        ...
        self.render_save_rate = render_save_rate
        self.env_id = None
    ...
    def reset(self, render_next=False):
        ...
        if self.store_gif and self.ep_no % self.render_save_rate == 0:
            ...
            self.fig, (self.ax1, self.ax2, self.ax3) = plt.subplots(1, 3, gridspec_kw={'width_ratios': [2,2, 1]}, sharex=True, sharey=True)
            self.ax3.axis("off")
    ...
    def render(self, state=None, mode='Non', tile_size=4):
        ...
        # print(objects_n)
        # object_img = np.repeat(COLORS_A_M, height, axis=0)
        # object_img = np.repeat(object_img, width, axis=1)

        object_img = np.tile(COLORS_A_M, (height,width,1))
        objects_loc = np.tile(OBJECT_ENCODING_M, (height,width,1))

        objects_new, holding_new = np.split(state, [len(OBJECTS)+1], axis=2)
        holding_new = np.concatenate((holding_new, np.zeros((height,width,6),dtype=int)), axis=2)
        # print(objects_new.shape, holding_new.shape)
        all_items = objects_new+holding_new
        # print(all_items)
        all_items = np.repeat(all_items, 3, axis=0)
        all_items = np.repeat(all_items, 3, axis=1)
        # print(all_items)
        # print(all_items.shape)
        # print("k", objects_loc.shape,all_items.shape)
        new_img = np.expand_dims(np.sum(all_items*objects_loc, axis=2),axis=2)
        new_img = new_img*object_img
        # print(new_img.shape)
        new_img = np.concatenate((new_img,np.zeros((3,width*3,3),dtype=int)),axis=0)
        # print("y",new_img.shape)
        # print(new_img)
        # objects_loc = np.repeat(objects_loc, width, axis=1)
        # print(object_img)
        ...
        if holding != 0:
            new_img[height*3:, 3:6] = 255
            # new_img[height*3:,3:6,:] = COLORS_N[holding]
    ...
    def render_edit(self, change_idxs):
        ...
        for x,y in change_idxs:
            ...
            objects_new, holding_new = self.obs_one_hot[x, y, :(len(OBJECTS) + 1)], self.obs_one_hot[x, y, (len(OBJECTS)+1):]
            holding_new = np.concatenate((holding_new, np.zeros(6, dtype=int)))
            # print(objects_new.shape, holding_new.shape)
            new_tile = objects_new + holding_new
            new_tile = np.repeat(np.expand_dims(new_tile, axis=1),9,axis=0).reshape(3,3,9)
            new_tile = np.expand_dims(np.sum(new_tile*OBJECT_ENCODING_M, axis=2),axis=2)*COLORS_A_M
            # print('g', new_tile.shape)
            # new_tile = self.obs_one_hot
            ...
            if (x, y) == self.agent_pos.tuple():
                ...
                self.obs_image[self.STATE_H * 3:, 3:6] = 255 if np.max(holding_new)!=0 else 0
                # holding_color = np.dot(self.obs_one_hot[x, y, 9:], COLORS_H2)
                # self.obs_image[self.STATE_H*3:,3:6,:] = holding_color
    ...
    def __render_gif(self, state_image=None, action_label=None, reward=404):
        ...
        patches = [mpatches.Patch(color=COLORS_rgba[i], label="{l}".format(l=OBJECTS_plus_agent[i])) for i in range(len(COLORS_rgba))]
        ...
        self.ax3.legend(handles=patches, bbox_to_anchor=(.5, 0.5), loc='center', borderaxespad=0.)
...
Desired Goals: {}""".format(self.ep_no, self.step_num, action_label, desired_goals)
        ...
        ttl = plt.text(0.00, .81, title_str, horizontalalignment='left',
                       verticalalignment='bottom', transform=self.ax.transAxes)
        txt = plt.text(0.00, 0.12, bottom_text, horizontalalignment='left',
